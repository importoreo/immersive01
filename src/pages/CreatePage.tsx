...
// Buy Gems Modal (간단 버전)
function BuyGemsModal({ isOpen, onClose, onPurchase }: { isOpen: boolean; onClose: () => void; onPurchase: (type: string, planId: string) => void; }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-white rounded-xl p-6 w-80 relative">
        ...
        <h2 className="text-lg font-bold mb-4">Buy Gems</h2>
        <div className="space-y-2">
          <button className="w-full py-2 bg-blue-500 text-white rounded" onClick={() => onPurchase('subscription', 'pro')}>Pro Monthly (1200 Gems) - $9.99</button>
          <button className="w-full py-2 bg-green-500 text-white rounded" onClick={() => onPurchase('onetime', 'big')}>Big Pack (1800 Gems) - $12.99</button>
        </div>
      </div>
    </div>
  );
...
function RefineExperienceParametersPage({ onBack, onNext, pillarsData }: { onBack: () => void; onNext: (data: ParametersData) => void; pillarsData: PillarsData }) {
  ...
  const experienceStyleOptions = getExperienceStyleOptions(pillarsData.framework);

  // Auto-select recommended option on mount
  React.useEffect(() => {
    ...
    <>
      ...
      <div className="absolute top-[75px] left-0 right-0 px-4">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-white font-semibold">Refine Vibe & Setting</h1>
        </div>
      ...
      <div className="absolute top-[155px] left-0 right-0 bottom-[180px] overflow-y-auto px-4">
        ...
        <div className="mb-8">
          <h2 className="text-white text-xl font-semibold mb-6">Experience Style</h2>
            ...
            {experienceStyleOptions.map((option) => (
              <div
                ...
                className={`p-4 rounded-xl cursor-pointer transition-all border-2 ${
                  selectedExperienceStyle === option.id
                    ? 'border-blue-400 bg-blue-400/20'
                    : 'border-white/20 bg-white/5 hover:bg-white/10'
                }`}
              >
                <div className="flex items-start gap-4">
                  {/* Preview Image - Fixed size and position */}
                  <div 
                    className="w-12 h-12 rounded-lg flex-shrink-0"
                    style={{
                      backgroundImage: option.previewImage,
                      backgroundSize: 'cover',
                      backgroundPosition: 'center',
                    }}
                  />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <h3 className={`font-medium ${selectedExperienceStyle === option.id ? 'text-blue-400' : 'text-white'}`}>
                        {option.title}
                      </h3>
                      {option.recommended && selectedExperienceStyle !== option.id && (
                        <span className="bg-orange-500/20 text-orange-400 text-xs px-2 py-0.5 rounded-full border border-orange-400/20">
                          Recommended
                        </span>
                      )}
                    </div>
                    <p className={`text-sm leading-relaxed ${selectedExperienceStyle === option.id ? 'text-blue-300/80' : 'text-white/60'}`}>
                      {option.description}
                    </p>
                  </div>
...
function InitialBlueprintQualityPage({ onBack, onGenerate, parametersData }: { onBack: () => void; onGenerate: (data: any) => void; parametersData: ParametersData }) {
  ...
  return (
    <>
          ...
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-purple-500/20 text-purple-400 text-sm rounded-lg border border-purple-400/30 hover:bg-purple-500/30 transition-colors"
              >
                Configure
              </button>
            </div>
      ...
      {/* Buy Gems Modal */}
      <BuyGemsModal isOpen={showBuyGemsModal} onClose={() => setShowBuyGemsModal(false)} onPurchase={() => setShowBuyGemsModal(false)} />
